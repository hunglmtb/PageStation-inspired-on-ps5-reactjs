...
const ReactplosiveModal = ({
  ...
  useEffect(() => {
    const timeout = setTimeout(() => setActive(isVisible), DEFAULTS.modalVisibilityTimeout)
    return () => clearTimeout(timeout)
  }, [isVisible])

  return (
    <div className={isVisible ? 'reactplosive-modal reactplosive-modal--active' : 'reactplosive-modal'}>
      <div
      ...
      >
        <button className="reactplosive-modal-close" onClick={() => onClose(false)}>
          <CloseIcon />
        </button>
        {title ? (
      ...
      <div className="reactplosive-modal-bg" onClick={() => (outsideClickEnabled ? onClose(false) : null)} />
    </div>
  )
}
